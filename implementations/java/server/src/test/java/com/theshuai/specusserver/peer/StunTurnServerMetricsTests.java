...
class StunTurnServerMetricsTests {
    ...
    void recordsRelayQueueDropsAndHighWaterMark() throws Exception {
        ...
        try {
            ...
            assertThat(meterRegistry.counter("specus.peer_mesh.turn.relay.queue.dropped").count())
                    .isEqualTo(1.0);
            assertThat(meterRegistry.get("specus.peer_mesh.turn.relay.queue.high.water").gauge().value())
                    .isEqualTo(1.0);
        } finally {
            release.countDown();
