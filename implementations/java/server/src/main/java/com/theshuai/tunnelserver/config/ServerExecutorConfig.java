...
public class ServerExecutorConfig {
    ...
    public ExecutorService loginExecutor(
            @Value("${tunnel.login.executor.core-size:8}") int coreSize,
            @Value("${tunnel.login.executor.max-size:32}") int maxSize,
            @Value("${tunnel.login.executor.queue-capacity:20000}") int queueCapacity) {
