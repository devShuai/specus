...
/**
 * Per-natural-month connection totals kept after the raw {@code specus_connection_record} detail is
 * archived. One row per (clientName, statMonth) where statMonth is {@code yyyy-MM}.
 */
@Entity
@Table(name = "specus_connection_stat",
        uniqueConstraints = @UniqueConstraint(
                name = "uk_specus_connection_stat",
                columnNames = {"tenant_id", "client_name", "stat_month"}
        ),
        indexes = {
                @Index(name = "idx_specus_connection_stat_tenant", columnList = "tenant_id"),
                @Index(name = "idx_specus_connection_stat_client", columnList = "client_name")
        })
@Getter
@Setter
