...
public class WebSocketTunnelHandler extends AbstractWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        handleAppFrame(session, WebSocketTunnelFrame.OPCODE_TEXT, message.isLast(), 0, 0, message.asBytes());
    }
    ...
    protected void handleBinaryMessage(WebSocketSession session, BinaryMessage message) throws Exception {
        ...
        buf.get(bytes);
        handleAppFrame(session, WebSocketTunnelFrame.OPCODE_BINARY, message.isLast(), 0, 0, bytes);
    }
    ...
    protected void handlePongMessage(WebSocketSession session, PongMessage message) throws Exception {
        ...
        payload.get(bytes);
        handleAppFrame(session, WebSocketTunnelFrame.OPCODE_PONG, true, 0, 0, bytes);
    }
    ...
    private void handleAppFrame(WebSocketSession session, int opcode, boolean finalFragment,
        ...
        do {
            ...
            byte[] framed = new WebSocketTunnelFrame(chunkOpcode, finalFragment && last,
    ...
    public void writeFrame(int streamId, byte[] payload) {
        ...
        try {
            WebSocketTunnelFrame frame = WebSocketTunnelFrame.decode(payload);
            if (frame.rsv() != 0) {
            ...
            synchronized (session) {
                switch (frame.opcode()) {
                    ...
                    case WebSocketTunnelFrame.OPCODE_CONTINUATION -> {
                        ...
                        if (original == WebSocketTunnelFrame.OPCODE_TEXT) {
                            session.sendMessage(new TextMessage(
                                    new String(framePayload, StandardCharsets.UTF_8), frame.finalFragment()));
                        } else {
                            session.sendMessage(new BinaryMessage(framePayload, frame.finalFragment()));
                    ...
                    case WebSocketTunnelFrame.OPCODE_PING ->
                            session.sendMessage(new PingMessage(ByteBuffer.wrap(framePayload)));
                    case WebSocketTunnelFrame.OPCODE_PONG ->
                            session.sendMessage(new PongMessage(ByteBuffer.wrap(framePayload)));
                    case WebSocketTunnelFrame.OPCODE_CLOSE -> session.close(new CloseStatus(
    ...
    private void detachBrowser(String channelId, CloseStatus status, boolean closeSession) {
        ...
        byte[] close = new WebSocketTunnelFrame(WebSocketTunnelFrame.OPCODE_CLOSE, true, 0,
