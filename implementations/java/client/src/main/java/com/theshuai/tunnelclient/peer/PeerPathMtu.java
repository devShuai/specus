...
/** Authenticated SPM2 packetization-layer MTU probe and per-path discovery state. */
final class PeerPathMtu {
    static final int MIN_INNER_MTU = 576;
    static final int MAX_INNER_MTU = 9_000;
    static final long PROBE_TIMEOUT_MILLIS = 750;
    static final long CACHE_TTL_MILLIS = 10 * 60_000L;
    ...
    static byte[] probe(long nonce, int innerMtu) {
        if (nonce <= 0 || innerMtu < Math.max(MIN_INNER_MTU, HEADER_BYTES)
                || innerMtu > MAX_INNER_MTU) {
            throw new IllegalArgumentException("invalid path MTU probe");
        }
    ...
    static byte[] ack(long nonce, int innerMtu) {
        if (nonce <= 0 || innerMtu < MIN_INNER_MTU || innerMtu > MAX_INNER_MTU) {
            throw new IllegalArgumentException("invalid path MTU acknowledgement");
        }
    ...
    static Message decode(byte[] payload) {
        ...
        if (type == TYPE_ACK && payload.length != HEADER_BYTES) {
            return null;
        }
        return type == TYPE_PROBE || type == TYPE_ACK
                ? new Message(type == TYPE_PROBE, nonce, innerMtu)
                : null;
    }
    ...
    private static void writeHeader(byte[] output, int type, long nonce, int innerMtu) {
        ...
        buffer.put(MAGIC).put((byte) type).putLong(nonce).putShort((short) innerMtu);
    }

    record Message(boolean probe, long nonce, int innerMtu) {
    }

    record Probe(long nonce, int innerMtu) {
    }

    record Transition(Probe probe, Integer completedMtu) {
        private static Transition idle() {
            return new Transition(null, null);
        }
    }

    /** RFC 8899-style search state. A path starts optimistic and becomes conservative after loss. */
    static final class Discovery {
        ...
        synchronized Transition activate(String nextPathKey,
            ...
            if (samePath && pendingSize > 0) {
                return Transition.idle();
            }
            if (samePath && validUntilMillis > nowMillis) {
                return Transition.idle();
            }
