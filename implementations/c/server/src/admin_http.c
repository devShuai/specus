...
static char *admin_build_polyfill_script(const char *prefix)
{
    ...
    int rc = admin_sb_appendf(
        ...
        "<script>(function(){try{"
        "var P='%s';"
        "function need(u){if(typeof u!=='string')return false;if(u.length===0||u.charAt(0)!=='/')return false;"
        "if(u.length>1&&u.charAt(1)==='/')return false;if(u.indexOf(P+'/')===0||u===P)return false;return true;}"
