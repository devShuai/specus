...
static char *admin_build_polyfill_script(const char *prefix)
{
    ...
    int rc = admin_sb_appendf(
        ...
        "<script>(function(){try{"
        "var P='%s';"
        "function need(u){if(typeof u!=='string'||!u)return false;var path=u;"
        "if(u.charAt(0)!=='/'){if(typeof location==='undefined')return false;var o=location.origin;"
        "if(!o||u.indexOf(o)!==0)return false;path=u.slice(o.length);if(!path||path.charAt(0)!=='/')return false;}"
        ...
        "function fix(u){if(!need(u))return u;if(u.charAt(0)==='/')return P+u;return location.origin+P+u.slice(location.origin.length);}"
        "if(typeof fetch==='function'){var of=fetch;window.fetch=function(i,n){try{if(typeof i==='string')i=fix(i);"
        "else if(i&&typeof i.url==='string'&&need(i.url))i=new Request(fix(i.url),i);}catch(e){}return of.call(this,i,n);};}"
        "if(typeof XMLHttpRequest!=='undefined'){var oo=XMLHttpRequest.prototype.open;XMLHttpRequest.prototype.open=function(m,u){"
        "try{u=fix(u);}catch(e){}arguments[1]=u;return oo.apply(this,arguments);};}"
