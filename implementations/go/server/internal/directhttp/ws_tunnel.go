...
var errWSTunnelClosed = errors.New("WS 隧道已关闭")
...
type WSClosedFunc func(*WebSocketTunnel)

// WebSocketTunnel 是 HTTP 直转通道的一条浏览器侧 WebSocket 隧道，对齐 Java WebSocketTunnelHandler：
...
func NewWebSocketTunnel(conn *websocket.Conn, streamID uint32, clientName string,
	sendData WSSendFunc, finish WSFinishFunc, onClosed WSClosedFunc) *WebSocketTunnel {
...
// StreamID returns the NAT stream identifier of this tunnel.
func (t *WebSocketTunnel) StreamID() uint32 { return t.streamID }

// ReadLoop 把浏览器 WS 消息封装成 SWS2 帧转发到客户端，直到任一端关闭后返回。
// 浏览器的 ping/pong 由 coder/websocket 自动处理，不进入隧道。
func (t *WebSocketTunnel) ReadLoop(ctx context.Context) {
	for {
...
func (t *WebSocketTunnel) sendAppFrame(ctx context.Context, opcode byte, payload []byte) error {
	offset := 0
	first := true
	for {
...
func (t *WebSocketTunnel) WriteFrame(ctx context.Context, data []byte) {
...
func (t *WebSocketTunnel) writeMessage(ctx context.Context, opcode byte, payload []byte) error {
...
func (t *WebSocketTunnel) CloseFromClient() {
...
func (t *WebSocketTunnel) closeFromBrowser(code websocket.StatusCode, reason string) {
	t.notifyOnce.Do(func() {
...
func (t *WebSocketTunnel) AddSendCredit(credit uint32) bool {
...
func (t *WebSocketTunnel) takeSendCredit(ctx context.Context, size int) bool {
...
func (t *WebSocketTunnel) signalWindow() {
	select {
