...
func newWSHarness(t *testing.T) *wsHarness {
	...
	h := &wsHarness{browser: browser, frames: make(chan []byte, 16), closed: make(chan *WebSocketTunnel, 1)}
	h.tunnel = NewWebSocketTunnel(serverConn, 7, "Demo client",
		...
		func(tunnel *WebSocketTunnel) { h.closed <- tunnel })
	t.Cleanup(func() {
		// 先断开浏览器侧，避免 tunnel.Close 的关闭握手空等对端回包。
		h.browser.CloseNow()
		h.tunnel.Close()
	})
	return h
...
// TestServeWebSocketOpenDataCloseLifecycle 覆盖完整生命周期（对齐 Java WebSocketTunnelHandler）：
...
func TestServeWebSocketOpenDataCloseLifecycle(t *testing.T) {
	...
	service := NewService(onlineRegistry("Demo client"), nil,
		func(clientName string, metadata map[string]any, conn *websocket.Conn) (*WebSocketTunnel, error) {
			openedCh <- metadata
			return NewWebSocketTunnel(conn, 1, clientName,
				func(frame []byte) error {
					frames <- frame
					return nil
				},
...
// TestWSTunnelChunksLargeBrowserMessage 大消息切成 maxSWS2Payload 的帧，
// 首帧保留原 opcode，后续为 CONTINUATION（对齐 Java handleAppFrame）。
func TestWSTunnelChunksLargeBrowserMessage(t *testing.T) {
	h := newWSHarness(t)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	go h.tunnel.ReadLoop(ctx)
...
// TestWSTunnelWritesClientFramesToBrowser 客户端 DATA 的 SWS2 帧还原成 WS 消息：
// 分片在 FIN 后重组为一条消息（与 Java 分片发送在浏览器端等价）。
func TestWSTunnelWritesClientFramesToBrowser(t *testing.T) {
	h := newWSHarness(t)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	h.tunnel.WriteFrame(ctx, first)
	h.tunnel.WriteFrame(ctx, last)
...
func TestWSTunnelClientCloseFrames(t *testing.T) {
	...
	case <-h.closed:
	case <-ctx.Done():
		t.Fatal("tunnel did not deregister after client CLOSE")
	}
...
// TestWSTunnelOrphanContinuationClosesTunnel 孤立 CONTINUATION 按非法帧处理（对齐 Java writeFrame）。
func TestWSTunnelOrphanContinuationClosesTunnel(t *testing.T) {
	h := newWSHarness(t)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	case <-h.closed:
	case <-ctx.Done():
		t.Fatal("tunnel did not deregister after orphan continuation")
	}
}

// TestWSTunnelSendCredit 验证与 HTTPStream 一致的窗口语义：
// 发送消耗 credit，WINDOW_UPDATE 按 outstanding 返还，非法增量被拒绝。
func TestWSTunnelSendCredit(t *testing.T) {
	h := newWSHarness(t)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	if h.tunnel.AddSendCredit(0) || h.tunnel.AddSendCredit(wsMaximumWindowBytes+1) {
		t.Fatal("zero/oversized credit must be rejected")
	}
	if h.tunnel.AddSendCredit(1) {
		t.Fatal("credit without outstanding bytes must be rejected")
	}

	if err := h.tunnel.sendAppFrame(ctx, sws2OpcodeText, []byte("hi")); err != nil {
		t.Fatalf("sendAppFrame failed: %v", err)
	}
	sent := h.nextFrame(t)
	frameBytes := sws2HeaderBytes + len(sent.payload)
	if h.tunnel.AddSendCredit(uint32(frameBytes) + 1) {
		t.Fatal("credit exceeding outstanding bytes must be rejected")
	}
	if !h.tunnel.AddSendCredit(uint32(frameBytes)) {
