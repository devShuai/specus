...
func TestNewLoggerWritesConfiguredFile(t *testing.T) {
	logPath := filepath.Join(t.TempDir(), "tunnel-server.log")
	t.Setenv("TUNNEL_LOG_FILE", logPath)
...
func TestNewLoggerRejectsRelativeFile(t *testing.T) {
	t.Setenv("TUNNEL_LOG_FILE", "relative.log")
	if _, _, err := newLogger(); err == nil {
		t.Fatal("relative TUNNEL_LOG_FILE was accepted")
	}
}
