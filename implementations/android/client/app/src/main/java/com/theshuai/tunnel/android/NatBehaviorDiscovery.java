...
final class NatBehaviorDiscovery {
    ...
    private ProbeRequest request(Probe probe) {
        ...
        return switch (probe) {
            case FILTER_CHANGE_IP_AND_PORT -> new ProbeRequest(
                    generation, probe, primaryEndpoint, otherEndpoint, true, true);
            case FILTER_CHANGE_PORT -> new ProbeRequest(
                    generation, probe, primaryEndpoint, primaryIpAlternatePort, false, true);
            case MAPPING_ALTERNATE_IP -> new ProbeRequest(
                    generation, probe, alternateIpPrimaryPort, alternateIpPrimaryPort, false, false);
            case MAPPING_ALTERNATE_IP_AND_PORT -> new ProbeRequest(
                    generation, probe, otherEndpoint, otherEndpoint, false, false);
        };
    }
    ...
    enum Probe {
        FILTER_CHANGE_IP_AND_PORT,
        FILTER_CHANGE_PORT,
        MAPPING_ALTERNATE_IP,
        MAPPING_ALTERNATE_IP_AND_PORT
    }
