...
/** Global and per-source limiter for unauthenticated UDP probe parsing. */
final class PeerUdpProbeRateLimiter {
    private static final long WINDOW_MILLIS = 1_000L;
    private static final long SOURCE_TTL_MILLIS = 60_000L;
    ...
    synchronized void cleanup(long nowMillis) {
        sourceWindows.entrySet().removeIf(
                entry -> nowMillis - entry.getValue().lastSeenMillis > SOURCE_TTL_MILLIS);
    }
    ...
    private void evictIfNeeded(long nowMillis) {
        ...
        while (iterator.hasNext() && sourceWindows.size() >= MAX_SOURCES) {
            ...
            if (nowMillis - entry.getValue().lastSeenMillis > SOURCE_TTL_MILLIS
                    || sourceWindows.size() >= MAX_SOURCES) {
                iterator.remove();
            }
        }
    }
    ...
    private static final class Window {
        ...
        Window(long startedMillis, int packets, long lastSeenMillis) {
