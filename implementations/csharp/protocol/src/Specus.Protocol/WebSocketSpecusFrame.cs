...
public sealed class WebSocketSpecusFrame
{
    ...
    public WebSocketSpecusFrame(byte opcode, bool finalFragment, byte rsv, ushort closeCode, byte[]? payload)
    {
        Opcode = opcode;
    ...
    public static WebSocketSpecusFrame Decode(ReadOnlySpan<byte> encoded)
    {
        ...
        try
        {
            return new WebSocketSpecusFrame(
                encoded[4],
                (flags & 1) != 0,
                (byte)((flags >> 1) & 7),
