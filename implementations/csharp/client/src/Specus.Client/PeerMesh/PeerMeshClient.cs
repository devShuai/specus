...
internal sealed class PeerMeshClient : IAsyncDisposable
{
    ...
    private bool ShouldRetryVirtualDeviceStartLocked() =>
        ...
        && !string.Equals(_config.PeerMeshDevice, SpecusClientConfig.DefaultPeerMeshDevice, StringComparison.OrdinalIgnoreCase)
    ...
    private async Task ReportDeviceAsync(SpecusRuntimeState runtime, FrameWriter writer, string status, string error, string natType, string endpoint, CancellationToken cancellationToken)
    {
    ...
    private static async Task SendPeerControlAsync(string toClientName, PeerControlMessage message, FrameWriter writer, SpecusRuntimeState runtime, CancellationToken cancellationToken)
    {
    ...
    private SpecusRuntimeState? Runtime()
    {
        lock (_sync)
        {
            return _runtime;
    ...
    private void PublishPeerMeshSnapshot()
    {
        ...
        SpecusPeerMeshSnapshot snapshot;
        lock (_sync)
        {
            var runtime = _runtime;
            snapshot = new SpecusPeerMeshSnapshot
            {
