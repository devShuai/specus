...
/// <summary>
/// Spins up a minimal in-process specus "server" that speaks just enough of the wire
...
/// </summary>
public class SpecusControlClientReconnectTests
{
    [Theory]
    ...
    public void ControlLoginFailureClassificationMatchesJava(
        ...
        int expected)
    {
        Assert.Equal((ControlLoginFailureAction)expected, SpecusControlClient.ClassifyControlLoginFailure(reason));
    }
    ...
    public async Task ClientLogsIn_Registers_RoundtripsData_AndReconnects()
    {
        ...
        var client = new SpecusControlClient(config, auth, new DirectHttpForwarder(http), NullLoggerFactory.Instance);
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    using (var stream = tcp.GetStream())
                    {
                        ...
                        Assert.True(read > 0);
                        var body = $$"""
                        {"tenantId":"default","clientId":1,"clientName":"csharp-tester","clientSessionId":1001,"accessToken":"cs-test-token","tokenTtlSeconds":3600,"nettyHost":"127.0.0.1","nettyPort":{{controlPort}},"maxOnlineInstances":2,"policy":{"enabled":true,"billingStatus":"ACTIVE","retryAfterSeconds":0},"specusConfigList":[{"port":9999,"specusAddress":"127.0.0.1","specusPort":{{upstreamPort}}}],"httpSpecusConfigList":[]}
                        """;
