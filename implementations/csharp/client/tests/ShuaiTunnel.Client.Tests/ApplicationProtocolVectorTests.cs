...
{
    [Fact]
    public void WebSocketTunnelFrameMatchesCentralSws2Vector()
    {
        ...
        var decoded = WebSocketTunnelFrame.Decode(expected);
        ...
        Assert.Throws<InvalidDataException>(() =>
            WebSocketTunnelFrame.Decode(Convert.FromHexString(vector.InvalidMagicHex)));
        Assert.Throws<InvalidDataException>(() =>
            WebSocketTunnelFrame.Decode(Convert.FromHexString(vector.TruncatedHex)));
        Assert.Throws<InvalidDataException>(() =>
            WebSocketTunnelFrame.Decode(Convert.FromHexString(vector.TrailingHex)));
    }
