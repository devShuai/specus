...
`Specus.Server.Data` holds the shared model: entities, `SpecusDbContext`, and value
...
Selected at runtime via `Specus:Database:Provider` (env `SPECUS_DB_PROVIDER`) plus the
`ConnectionStrings:Specus` connection string (env `SPECUS_CONNECTIONSTRINGS_SPECUS`):

| Provider value            | EF package                                  | Migrations assembly                  |
|---------------------------|---------------------------------------------|--------------------------------------|
| `sqlite` (default)        | `Microsoft.EntityFrameworkCore.Sqlite`      | `Specus.Server.Data` (this proj)|
| `postgres`/`postgresql`   | `Npgsql.EntityFrameworkCore.PostgreSQL`     | `Specus.Server.Data.Postgres`   |
| `mysql`/`mariadb`         | `MySql.EntityFrameworkCore` (Oracle)        | `Specus.Server.Data.MySql`      |

> MySQL uses Oracle's official provider because Pomelo has no EF Core 10 build yet.

Connection string examples:

- PostgreSQL: `Host=...;Database=specus;Username=...;Password=...`
- MySQL: `server=...;database=specus;user=...;password=...`
...
dotnet ef migrations add <Name> -p src/Specus.Server.Data         -s src/Specus.Server.Data         -o Migrations
dotnet ef migrations add <Name> -p src/Specus.Server.Data.Postgres -s src/Specus.Server.Data.Postgres -o Migrations
dotnet ef migrations add <Name> -p src/Specus.Server.Data.MySql    -s src/Specus.Server.Data.MySql    -o Migrations
```
...
- `specus_resource_traffic_usage`
- `specus_http_traffic_exchange`
- `specus_tcp_traffic_frame`

本轮消息/互传模型还为 `specus_client_session` 增加发送、接收、附件、媒体预览和附件大小能力字段，并新增
...
`AddManagementRegistration` 增加 `specus_management_user_email` 与
`specus_management_registration_challenge`，用于邮箱唯一绑定和短期注册验证；验证码只保存由本地 JWT
...
`specus_traffic_usage` now includes Java-aligned `tenant_id`. Startup adds the column and
`idx_specus_traffic_tenant` to existing databases, traffic flush writes the owning client's tenant,
...
`specus_connection_record` keeps `tenant_id` nullable in the EF model so old rows remain readable
while startup compatibility SQL backfills from `specus_client_account`. The current snapshots match
that compatibility shape.

`specus_connection_stat` now includes Java-aligned `tenant_id`. Startup adds the column and
`idx_specus_connection_stat_tenant` to existing databases, then backfills old monthly rows by
...
`Specus:Elasticsearch:Uris` / `SPECUS_ELASTICSEARCH_URIS` is blank, detail records stay in the
database. When configured, HTTP/TCP detail records are indexed into
`specus-http-traffic` / `specus-tcp-traffic` by default, management queries read from
