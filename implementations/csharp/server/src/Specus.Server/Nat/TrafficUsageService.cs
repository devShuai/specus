...
{
    public const string ResourceTypeTcpSpecus = "TCP_SPECUS";
    ...
    public void RecordTcpUpload(string? clientName, int listenPort, long bytes)
    {
        ...
        {
            RecordResourceUpload(clientName, ResourceTypeTcpSpecus, TcpKey(listenPort), bytes);
        }
    }
    ...
    public void RecordTcpDownload(string? clientName, int listenPort, long bytes)
    {
        ...
        if (!string.IsNullOrEmpty(clientName) && listenPort > 0 && bytes > 0)
        {
            RecordResourceDownload(clientName, ResourceTypeTcpSpecus, TcpKey(listenPort), bytes);
        }
    }
    ...
    private static async Task<(long? ResourceId, string ResourceName)> ResolveResourceAsync(SpecusDbContext db,
        ClientAccount account, ResourceCounterKey key, CancellationToken cancellationToken)
    {
        if (key.ResourceType == ResourceTypeTcpSpecus)
        {
            var listenPort = ParseTcpKey(key.ResourceKey);
            var mapping = await db.SpecusMappings.AsNoTracking()
