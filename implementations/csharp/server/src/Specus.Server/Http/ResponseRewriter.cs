...
internal static partial class ResponseRewriter
{
    ...
    private static string BuildPolyfillScript(string prefix)
    {
        ...
        return "<script>(function(){try{"
               + $"var P='{escaped}';"
               + "function need(u){if(typeof u!=='string'||!u)return false;var path=u;if(u.charAt(0)!=='/'){if(typeof location==='undefined')return false;var o=location.origin;if(!o||u.indexOf(o)!==0)return false;path=u.slice(o.length);if(!path||path.charAt(0)!=='/')return false;}if(path.length>1&&path.charAt(1)==='/')return false;if(path.indexOf(P+'/')===0||path===P||path.indexOf(P+'?')===0||path.indexOf(P+'#')===0)return false;return true;}"
