...
public sealed class AdminApiTests : IAsyncLifetime
{
    ...
    public async Task ClientCrudAndCredentialCrudMatchJavaApi()
    {
        ...
        var createSpecus = await client.PostAsJsonAsync($"/api/admin/clients/{created.Client.Id}/specus-mappings", new
        {
            listenPort = 45998,
        ...
        Assert.Equal(HttpStatusCode.Created, createSpecus.StatusCode);
    ...
    public async Task ManagementUsersAndOwnerScopeMatchJavaApi()
    {
        ...
        var createBlockedSpecus = await alice.PostAsJsonAsync(
            $"/api/admin/clients/{adminClient.Client.Id}/specus-mappings", new
            {
                listenPort = 46101,
                targetAddress = "127.0.0.1",
                targetPort = 8080,
        ...
        Assert.Equal(HttpStatusCode.Forbidden, createBlockedSpecus.StatusCode);

        var createOwnSpecus = await alice.PostAsJsonAsync(
            $"/api/admin/clients/{aliceClient!.Client.Id}/specus-mappings", new
            {
                listenPort = 46102,
                targetAddress = "127.0.0.1",
                targetPort = 8080,
        ...
        Assert.Equal(HttpStatusCode.Created, createOwnSpecus.StatusCode);
    }

    [Fact]
    public async Task SpecusCrudAndOfflineNatPushBehaveLikeJavaApi()
    {
        using var client = await AuthenticatedClientAsync();
        var demo = await ReadDemoClientAsync(client);

        var create = await client.PostAsJsonAsync($"/api/admin/clients/{demo.Id}/specus-mappings", new
        {
            listenPort = 45123,
            targetAddress = "127.0.0.1",
            targetPort = 8080,
        ...
        Assert.Equal(HttpStatusCode.Created, create.StatusCode);
        var specus = await create.Content.ReadFromJsonAsync<SpecusBody>(JsonOptions);
        Assert.NotNull(specus);
        Assert.Equal(45123, specus!.ListenPort);
        Assert.True(specus.DetailCaptureEnabled);

        var list = await client.GetFromJsonAsync<List<SpecusBody>>(
            $"/api/admin/specus-mappings?clientId={demo.Id}", JsonOptions);
        Assert.Contains(list!, row => row.Id == specus.Id);

        var update = await client.PutAsJsonAsync($"/api/admin/specus-mappings/{specus.Id}", new
        {
            listenPort = 45124,
        ...
        update.EnsureSuccessStatusCode();
        var updated = await update.Content.ReadFromJsonAsync<SpecusBody>(JsonOptions);
        Assert.NotNull(updated);
        ...
        var delete = await client.DeleteAsync($"/api/admin/specus-mappings/{specus.Id}");
        Assert.Equal(HttpStatusCode.NoContent, delete.StatusCode);
    }
    ...
    [Fact]
    public async Task DirectHttpSpecusRejectsOfflineClientAndOversizedBody()
    {
        using var admin = await AuthenticatedClientAsync();
        var demo = await ReadDemoClientAsync(admin);
        var create = await admin.PostAsJsonAsync($"/api/admin/clients/{demo.Id}/http-routes", new
        ...
        var failingContext = new SpecusConnectionContext(
    ...
    private sealed record ClientDetailBody(ClientBody Client, List<SpecusBody> SpecusMappings, List<HttpRouteBody> HttpRoutes);
    ...
    private sealed record SpecusBody(
        long Id,
        long ClientId,
        string ClientName,
        int ListenPort,
