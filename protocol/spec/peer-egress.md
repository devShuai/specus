...
命中 `egress` 规则而**出口不可用**（离线、尚未在线、流被清掉）的流不是静默丢弃，而是像一台不可达的主机那样回应应用，让它立刻失败、自行重试，而不是等自己的超时——TCP 连接要一分钟以上，而规则其实在正常工作。三端写回本机 TUN 的字节完全一致，由 `peer-egress-failure-v1.json` 钉住：

- **消费端拒绝转发的 TCP 段**按 RFC 793 回 RST，与出口用户态栈同一条规则：带 ACK 的段，复位的序号取它的确认号；不带 ACK 的段（新连接的 SYN），回 RST|ACK，确认号覆盖它的序号空间。应用看到的是「连接被拒绝」。
- **消费端自己清掉的流**（出口离线、规则改动使它不再指向这个出口）没有可回应的段，复位从消费端记住的东西造出来：每个 TCP 流记着应用最后一次的确认号（它的接收窗口起点，现代协议栈按 RFC 5961 只在这个序号上接受复位）和它发到的位置。RST|ACK，序号取前者，确认号取后者。应用只发过 SYN 的流没有可用的序号，不主动复位，它重传的 SYN 到达时按上一条回应。
- **消费端拒绝转发的 UDP 数据报**回 ICMP 目的不可达、主机不可达（类型 3 代码 1），引用原包的 IP 头和前 8 字节，协议栈靠它把错误对到 socket 上。被清掉的 UDP 流不主动通知，下一个数据报到达时回应。
- **不回应的情况**：发送失败（`send-failed`）——会话可能正在重建，那条流可能还能恢复，静默丢弃让它有机会；`block` 规则——它是丢弃，不是拒绝；不承载的协议。
...
- **出口到消费端方向没有发送侧流控，超过接收端缓冲的响应会失败而不是变慢。** 出口的用户态 TCP 栈对外通告固定的 65535 字节接收窗口，不做窗口缩放；发送方向不看消费端通告的窗口，没有拥塞窗口，也不对真实 socket 施加背压——从目标读到多少就立即切段发给消费端。RTT 估算只用来计算重传超时（初始 1 s，下限 200 ms，上限 60 s，单段最多重传 6 次）。Peer Mesh 发送队列满时丢帧（日志 `dropped peer data frame, data plane saturated`），靠超时重传恢复，而重传仍按原速发进同一个满窗口。后果不是吞吐退化：超出接收端能容纳的量之后，段被持续丢弃，单段重传 6 次耗尽（指数退避约 113 秒）后栈复位连接，应用收到 `Connection reset by peer`。[真机实验室](../../scripts/peer-egress-lab/README.md)实测，环回目标、1280 MTU、无丢包：4 KiB / 64 KiB / 256 KiB 均在毫秒级完整到达，512 KiB 只到 8.6% 后复位，1 MiB 只到 46% 后复位；同方向**上传** 8 MiB 有 7.3 MiB/s，直连下载 1523 MiB/s。天花板随接收端 socket 缓冲自动调整浮动，同一份代码在另一台机器上 512 KiB 能过、1 MiB 不能。加 2% 双向丢包后 256 KiB 也过不去。修复见 #74。
...
- **控制连接重连时消费端自己撤掉全部路由，窗口内同样会静默直连。** 重连走完整重启路径：撤回本功能拥有的全部条目、关闭虚拟网卡（指向它的路由随之被内核删除），重连成功后再重算装回。真机实验室测到这个窗口约 2 秒，期间命中规则的请求从消费端自己的地址直达目标。撤销对**真正退出**是对的（留下的路由指向一条已无人服务的隧道，是黑洞），对**重启**是错的。控制连接重连在正常运行中随时会发生，所以这不是罕见路径。上一条说的是别人改了你的路由表，这一条是本功能自己撤的。修复见 #73。
