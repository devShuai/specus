...
出口仍发送 TCP RST；消费端收到 `flow-reject` 时，必须先核对完整流元组和该流绑定的出口设备。
非所属出口、未知流与重复拒绝不得删除流、回注复位或增加拒绝计数。对匹配的已建立 TCP 流，消费端
...
传输：若 RST 与拒绝消息均丢失，应用仍可能依赖后续重传或超时；收到一条拒绝消息本身不能再阻碍复位。
