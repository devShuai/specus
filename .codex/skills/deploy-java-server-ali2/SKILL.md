...
description: Current shuai-tunnel project only. Use when the user asks to package/build the Java tunnel-server and push/deploy it to ali2, including requests like "打包server，推到ali2", "更新服务端到ali2", or "部署 Java server 到 ali2". Also deploy the OpenResty-served admin frontend for tunnel.devshuai.com. This skill is not for other repositories or other deployment targets.
---
...
scp -r deploy/openresty ali2:/tmp/shuai-tunnel-openresty
scp -r apps/admin-web/dist ali2:/tmp/shuai-tunnel-admin-web-dist
ssh ali2 "sudo bash /tmp/shuai-tunnel-java-systemd/update.sh /tmp/tunnel-server.jar"
ssh ali2 "sudo ADMIN_WEB_DIST=/tmp/shuai-tunnel-admin-web-dist bash /tmp/shuai-tunnel-openresty/install-admin-web.sh && sudo openresty -s reload"
```
...
The OpenResty script installs `apps/admin-web/dist` into `/opt/shuai-tunnel/admin-web`, fixes static file permissions for the OpenResty worker, installs `deploy/openresty/shuai-tunnel.conf` as `/usr/local/openresty/nginx/conf/conf.d/tunnel.devshuai.com.conf` by default when that directory exists, runs `openresty -t`, and then the deployment command reloads OpenResty. Override `OPENRESTY_CONF_NAME` only when deploying to a different config file. The current config is for `tunnel.devshuai.com` and proxies dynamic requests to `127.0.0.1:8088`.
...
ssh ali2 "openresty -t && curl -k -I https://tunnel.devshuai.com/ && curl -k -I -H 'Accept-Encoding: gzip' https://tunnel.devshuai.com/assets/$(ls /opt/shuai-tunnel/admin-web/assets | grep '^index-.*\.js$' | head -n 1)"
```
...
- OpenResty config lives under `deploy/openresty` locally and normally installs to `/usr/local/openresty/nginx/conf/conf.d/tunnel.devshuai.com.conf` on ali2. Do not overwrite certificates under `/usr/local/openresty/nginx/certs`.
- OpenResty static root is `/opt/shuai-tunnel/admin-web`; it is safe for `install-admin-web.sh` to replace this static directory with the freshly built `dist`.
