...
Run an end-to-end echo workload through a public TCP tunnel. The default levels are the protocol audit's
...
go run .\tools\loadtest\tcp_stream_load.go `
  -addr tunnel.example.com:19000 `
  -duration 60s `
  -output .\tcp-capacity.json
```

The target must be a byte-for-byte TCP echo service behind the tunnel. Use `-slow-read 250ms` to exercise
...
go run ./tools/loadtest/tcp_stream_load.go -addr tunnel.example.com:19000 -duration 60s
...
java -jar implementations/java/client/target/tunnel-client-1.0.0-SNAPSHOT-benchmarks.jar PeerDataFrameCodecBenchmark
