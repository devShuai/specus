...
tag 构建统一注入版本到 Go `-ldflags`、Java Maven revision、.NET `Version` 与 Android `versionName`，并在打包前执行对应测试。Java root/common/client/server/STUN 共用 `${revision}${changelist}`：本地构建使用下一个补丁版的 `-SNAPSHOT`，Release 流水线用 `-Drevision=<tag without v> -Dchangelist=` 生成统一正式版本，并校验 client/server/STUN Manifest。Maven target 使用不含版本的稳定文件名，GitHub Release 归档再按 tag 重命名；flattened POM 用于将来的 Nexus install/deploy。正式 Tag 发布会先校验根 POM 的默认 `revision` 与 Tag 一致；手动 workflow 以 commit 试跑时则映射为合法的 `0.0.0-commit.<hash>` 构建版本，归档名仍保留原 commit。Android `versionCode` 使用单调递增的 workflow run number。APK 先在不接触密钥的构建任务中测试并产出 unsigned 内部 artifact，再由独立的 `release-signing` 受保护环境签名；签名任务必须同时匹配固定的 `SPECUS_ANDROID_CERT_SHA256`，unsigned artifact 不进入 GitHub Release。缺少签名 secrets 或证书指纹不匹配时流水线直接失败。所有归档与 APK 同时生成名称不冲突的 SHA-256 sidecar/清单，GitHub Release 继续作为默认外链分发和灾备来源。
