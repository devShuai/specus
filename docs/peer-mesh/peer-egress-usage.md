...
- **控制连接断开重连时也有同样的窗口，约 2 秒。** 客户端重连走完整重启路径，会先撤掉自己装的全部路由，重连成功后再装回；这期间命中规则的流量同样从本机出去。日志里的 `control connection closed` 后面紧跟 `routes added` 就是一次这样的窗口。网络抖动、服务端重启、切网都会触发。修复跟踪在 #73。
