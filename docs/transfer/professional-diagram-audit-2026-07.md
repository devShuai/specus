...
本轮按本文优先级从 P0 到 P3 顺序完成修复。下表是当前实现的状态索引；“已修复”表示代码路径与回归检查均已覆盖，“已优化”表示已消除本文指出的主要热点并保留后续性能专项的扩展空间。

| 编号 | 状态 | 处理与核对依据 |
|---|---|---|
| F1 | ✅ 已修复 | 结构性导入、恢复、云端加载和 embed 替换统一取消挂起同步，并在替换前 flush；写回前校验活动页面仍存在。 |
| F2 | ✅ 已修复 | 远端 update 到达时先处理本地挂起事务再合并并重绘，避免旧 Graph 覆盖远端新增或恢复已删除内容。 |
| F3 | ✅ 已修复 | 改为持久化 Y.Doc 镜像的增量校验器，只检查变化集合并执行文档全局上限校验，不再为每条 update 完整复制文档。 |
| F4 | ✅ 已修复 | 发送接口返回 `boolean/Promise<boolean>`；失败时显示同步中断并按退避策略重发全量状态。 |
| F5 | ✅ 已修复 | draw.io 使用流式解压与输出上限；VSDX 同时限制单 entry 和总解压量，并补充超限回归测试。 |
| F6 | ✅ 已修复 | 节点与 draw.io 导入导出均以 `bold === true` 判定粗体，缺省值恢复为常规字重。 |
| F7 | ✅ 已修复 | 会话版本改存 `sessionStorage`，按画板 key 隔离，最多 20 条，并在配额不足时保留最新可写版本。 |
| F8 | ✅ 已修复 | 无 key 的 embed 页面使用随机临时画板 key，不再共享 `embed:default` 缓存。 |
| F9 | ✅ 已修复 | 评论、页面复制、模板插入和连线创建均在本地入口强制执行节点、连线、页面及评论上限。 |
| F10 | ✅ 已修复 | 新页面 order 改为当前最大值加一；文档校验允许安全范围内的单调 order，并拒绝异常大值。 |
| F11 | ✅ 已修复 | 远端选中单元现在绘制协作者颜色轮廓，并显示成员名称。 |
| F12 | ✅ 已修复 | 流程图与白板事件拆分队列；流程图 update 使用完整内容哈希去重，不再依赖同毫秒前缀。 |
| F13 | ✅ 已优化 | `renderFromDocument` 改为按 cell id 增量增删改并保留选区；页面/评论仅在对应 Y 类型变化时刷新。 |
| I1 | ✅ 已修复 | 锁定节点不可拖动、缩放、旋转、编辑、删除或经属性栏修改；混合选区跳过锁定元素并反馈数量，仍可直接解锁。 |
| I2 | ✅ 已修复 | 快捷键监听提升到当前编辑器活动期的 `window`，保留输入框和可编辑元素守卫。 |
| I3 | ✅ 已修复 | 画布拦截 Ctrl/Meta+滚轮并执行 Graph 缩放，阻止浏览器页面误缩放，同时补齐缩放快捷键。 |
| I4 | ✅ 已修复 | 撤销/重做提供明确反馈；跨页事务会自动切到受影响页面并说明结果。 |
| I5 | ✅ 已修复 | 文字、数字和颜色字段在聚焦编辑时保留本地草稿，仅在切换选中对象或结束编辑后同步模型。 |
| I6 | ✅ 已修复 | 只读状态在现有 Graph 上动态生效；全屏仅改变布局，不再因二者切换销毁画布，选区与视图得以保留。 |
| I7 | ✅ 已修复 | Escape 按“弹层/协作面板 → 右键菜单 → 移动侧栏 → 选区 → 全屏”的优先级处理。 |
| I8 | ✅ 已修复 | 右键菜单 portal 到 viewport、按真实边界夹紧，支持首项聚焦和 Escape 回焦；空白右击会清理旧选区。 |
| I9 | ✅ 已修复 | 监听 scale、translate 与滚动变化更新视图 epoch，远程光标和选区随本地视图同步移动。 |
| I10 | ✅ 已修复 | 复制同时写入 maxGraph 与系统剪贴板；无 Graph 选区时不阻止浏览器默认复制。 |
| I11 | ✅ 已修复 | 状态反馈在手机端持续可见，错误另有画布横幅，不再只剩无文本圆点。 |
| I12 | ✅ 已修复 | 画布提示明确标注 Ctrl+Shift 拖拽平移、Ctrl+滚轮缩放和连线折点操作。 |
| I13 | ✅ 已修复 | 取消每条远端 update 刷写状态栏，仅保留有意义的连接与操作反馈。 |
| I14 | ✅ 已修复 | presence 使用协作成员显示名与中文文案，并在画布上绘制远端选区轮廓。 |
| I15 | ✅ 已修复 | 内部剪贴板为空时粘贴会明确提示，不再静默无响应。 |
| I16 | ✅ 已修复 | 快捷键标签按平台展示 Ctrl 或 Command 语义。 |
| I17 | ✅ 已修复 | 支持 Ctrl/Command+加号、减号、0，以及 Shift+1 适应画布。 |
| I18 | ✅ 已修复 | 编辑菜单、右键菜单与键盘均支持剪切，并遵守锁定与只读约束。 |
| I19 | ✅ 已修复 | 评论关联页面或元素已删除时提供明确定位失败反馈。 |
| I20 | ✅ 已修复 | 点阵大小和偏移随 Graph scale/translate/scroll 更新，与吸附网格保持一致。 |
| I21 | ✅ 已修复 | 重命名与版本命名输入框聚焦时自动全选原值。 |
| U1 | ✅ 已修复 | 专业编辑器范围内的 cyan 强调、焦点环、加载 spinner 与占位样式统一为 Apple 蓝 token。 |
| U2 | ✅ 已修复 | 成功、危险、当前态统一使用 `--diagram-apple-success/danger/blue` 及对应 soft token。 |
| U3 | ✅ 已修复 | 暗色画布与网格颜色提升为根级主题 token，组件不再内联第三套灰色。 |
| U4 | ✅ 已修复 | 账号、云文档与通用编辑弹窗统一表面、圆角、页眉页脚和主题 token，portal 在明暗模式下均可取值。 |
| U5 | ✅ 已修复 | 工具栏下拉复用 Apple 右键菜单表面和交互色，菜单项使用一致的中等圆角。 |
| U6 | ✅ 已修复 | 节点/连线统计、远端选择标签和状态文案统一中文，移除裸 peerId。 |
| U7 | ✅ 按规范处理 | 现行全站设计规范要求字距为 0，因此保留统一规则，同时移除编辑器内不会生效的 tracking 类，消除死样式。 |
| U8 | ✅ 已修复 | 状态、说明、时间和图形详情的最小字号提升到 10px，并提高低对比文字层级。 |
| U9 | ✅ 已修复 | 评论、版本和云文档日期统一通过 `formatDiagramTimestamp` 输出短格式。 |
| U10 | ✅ 已修复 | 状态区区分 info/success/error；错误使用 danger token，并在画布顶部显示可关闭警示。 |
| U11 | ✅ 已修复 | 云文档加载增加统一 spinner，版本增加空状态，图形库重复空结果合并。 |
| U12 | ✅ 已修复 | 独立流程图 titlebar 注入公共工具菜单；主题按当前产品约定保留在“视图”菜单。 |
| U13 | ✅ 已修复 | `--diagram-apple-*` 从根级 `--app-apple-*` 派生，减少双 token 漂移。 |
| U14 | ✅ 已修复 | 桌面与移动侧栏统一为图库 260px、属性 300px。 |
| U15 | ✅ 已修复 | 图库、属性与协作计数统一使用 `diagram-apple-count-badge`。 |
| U16 | ✅ 已修复 | 评论/版本 Tab 内移除重复标题，只保留紧凑操作入口。 |
| U17 | ✅ 已修复 | 非全屏编辑器高度改为响应式 `clamp`，移除矮视口下固定 540px 溢出。 |

额外完成窄屏核对：手机与平板端把公共工具和账号入口固定在标题右侧，编辑菜单仍可横向访问，页面本身不产生横向溢出。

### 最终核对

* `npm test`：21 个测试文件、149 项测试全部通过。
* `npm run typecheck`：通过。
* `npm run build`：通过；仅保留 maxGraph 上游 `eval` 与既有大 chunk 提示。
* 浏览器实测：桌面 1440×900、平板 820×1180、手机 390×844；覆盖明暗主题、模板插入、连线属性、三阶贝塞尔控制点、锁定保护、只读切换、右键菜单边界、移动侧栏、弹窗、系统剪贴板与设计栏恢复入口。干净启动页无前端 console error。
