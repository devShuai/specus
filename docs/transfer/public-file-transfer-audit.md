...
> 2026-07-20 权限策略更新：免登录能力限定为 WebRTC Direct 和认证 TURN；OSS 预签名上传、完成和下载均要求有效账号 Bearer JWT，并继续执行原有房间 Token/角色校验。本文后续关于公开附件 `permitAll` 的文字仅用于保留历史审计上下文。
>
> 2026-07-20 额度策略更新：公开互传和管理消息附件共用登录账号额度；有效 OSS 存储上限为 1 GiB，按 UTC 自然月签发的下载流量上限为 1 GiB。上传完成会用 HEAD 实际大小复核，下载流量按每次成功签发 URL 的文件完整大小计费。
