...
Java `mvn -Dspecus.server.web.skip=true -pl implementations/java/server -am test`（specus-common 43 + specus-server 130）通过。
...
| 编号 | 状态 | 子系统 | 当前结论 |
| --- | --- | --- | --- |
| S-1 | ALIGNED | Direct HTTP WebSocket 隧道 | Go 已补齐 `/http/**` WS upgrade 与 12 字节 SWS2 帧隧道（`directhttp/sws2.go`、`ws_specus.go`），含流控与 16MB 读上限，生命周期与 Java `WebSocketSpecusHandler` 逐条对应。 |
...
配置分散在 `specus.tls.*`（控制通道/Netty）和 `server.ssl.*`（管理 HTTP/Tomcat）两处；Go 单配置覆盖两端且支持 PEM。
...
| T-5 | ALIGNED | DB 表集合 | 5 张房间/流程图表已补齐到全部 3 个 schema，`specus_websocket_ticket.room_role` 纳入启动列迁移。 |
...
| T-8 | ALIGNED | 413 预检 | Java `HttpSpecusBodyLimitFilter` 已在 Content-Length 短路时记录 413 交换，与 Go 行为一致。 |
...
- **Elasticsearch**：`SPECUS_ELASTICSEARCH_URIS`、DB->ES 后端切换、index ensure、100GB/10GB 清理、
