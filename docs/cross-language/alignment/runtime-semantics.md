...
- C server 的 Direct HTTP/WS v2 bridge 已进入完整门禁：`OPEN/DATA/FIN/RST/WINDOW_UPDATE`、SWS2 shared vectors、continuation/ping/pong/close、query 大括号编码、route 鉴权、HTML/CSS 改写和有界 gzip/deflate 均由单测与 Java client E2E 覆盖。mapping/route mutation 会热推权威全集，手工 push 在线 `200`、离线 `409`。
- C server 已补可选 Elasticsearch HTTP/TCP 明细写入、查询和容量清理；未配置时继续使用 SQLite。逐 route 媒体采集已支持专用 S3-compatible/RustFS 校验、HLS/DASH/渐进式对象、multipart、Range 去重与跨对象连续区间、incomplete 稀疏回放、可选回源、tenant ticket、manifest/asset 和定时过期清理。公共 discovery 已通过 C↔C 双实例和 C↔Java Redis roster/名称/双向信令混部门禁。

## 阶段 4：Peer Mesh 控制面与数据面

状态：Go、.NET、C server 的控制面和标准 STUN/TURN relay 已对齐 Java；Go client 与 .NET client 已补齐 Linux TUN / Windows Wintun / macOS utun、X25519/HKDF/AES-GCM frame 与 UDP 数据面接入；Android client 已完成控制通道、VpnService、加密 UDP direct/TURN relay 的源码与 JVM 协议测试，真机验收仍单列保留。
...
- C server：
  ...
  - C server 不提供 C client 或虚拟网卡；Java/Go/.NET/Android client 负责 SPM2 加密数据、TUN/VPN、direct 打洞与 relay fallback。真实跨 NAT、真机 VPN/TUN 和长流量仍属于发布环境门禁。
