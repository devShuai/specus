...
>
> 状态复核：2026-07-10，基于 `main` 的当前实现。`✅` 表示代码能力已落地；`⚠️` 表示主体已落地但仍有原验收项未完成。
...
| Sprint 1 | 服务端 hot path + 流量持久化 | 2 天 | ✅ 已完成 |
| Sprint 2 | 导航重组 + 流量面板拆分 + 空状态 | 4 天 | ✅ 已完成 |
| Sprint 3 | 私有组网面板重构 + 客户端详情抽屉 + 双向同时打洞 | 4 天 | ⚠️ 部分完成（详情抽屉仍缺 peer 会话/重试打洞） |
| Sprint 4 | RTT 感知选路 + TCP 采样 + 日志降级 + ACL 方向性 + 窄屏 SVG | 3 天 | ⚠️ 主体完成（TCP 尾帧/异常帧无单独保留语义） |
...
- `keepaliveDirectPaths()` 由独立 5s 周期检查调度；每条健康 direct 会话约每 25s 发送一次保活，当前保活与普通探针一样使用 3 包突发
...
## 3. Sprint 1 — 服务端 hot path + 流量持久化（已完成）
...
- 提交：`50af83f`（含并行会话的流量捕获/peer-mesh WIP）
- 部署到 ali2

---

## 4. Sprint 2 — 导航重组 + 流量面板拆分 + 空状态（已完成）
...
## 5. Sprint 3 — 私有组网面板重构 + 客户端详情抽屉 + 双向同时打洞（部分完成）
...
当前实现已提供基本信息、在线状态、流量、TCP/HTTP 映射和「强制刷新端口映射」；尚未在抽屉中展示 peer 会话，也没有「重试打洞」动作。
...
当前通过触发式双向探测落地：收到入站探针且 direct 尚未建立时，立即反向扫描对端候选；探针突发沿用 S0.3 的节流。

验收：peer 直连成功率在复 NAT 网络下提升；面板信息密度下降；抽屉可触发运维动作。

---

## 6. Sprint 4 — RTT 感知选路 + TCP 采样 + 日志降级 + ACL 方向性 + 窄屏 SVG（主体完成）
...
- Sprint 2：✅ 完成
- Sprint 3：⚠️ Peer Mesh 面板与触发式双向探测已完成；客户端详情抽屉仍缺 peer 会话与重试打洞
- Sprint 4：⚠️ RTT、采样开关、日志、ACL 方向和窄屏适配已完成；TCP 尾帧/异常帧无单独保留语义
