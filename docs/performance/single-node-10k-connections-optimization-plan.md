...
>
> 历史基线：本文的“现状”描述来自 `57f1a67`（2026-06-16），不是 2026-07-10 当前实现状态。
> 此后共享外部监听 EventLoop、连接预算、写缓冲水位、最大帧、流量 UPSERT/批量 flush 与 TCP 采样等
> 多项已落地；当前进度以 `docs/unified-improvement-plan.md` 和源码为准。本文保留为容量优化清单与压测口径。
