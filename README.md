...
公开的 [Specus Client 下载页](https://specus.devshuai.com/download) 会优先读取服务端版本编目，按访问设备推荐 macOS Homebrew、Windows 桌面版、Android APK 或对应架构的 Go 客户端；默认编目可直接指向 GitHub Release，内网/离线场景也可使用服务端托管包，并可展开查看全部平台与实现。

管理后台「系统 → 客户端发布」可登记带 SHA-256/文件大小的 GitHub Release 资产，也可上传由服务端计算摘要和大小的托管包；两者都按 `实现 / 平台 / 架构 / 版本` 编目，并原子切换同一目标的最新版本。Go 与 .NET 客户端支持提示后确认自更新及校验失败回滚；Java 与 Android 只提示并打开受校验的下载地址，不静默替换程序。客户端在启动时检查；Go、Java 与 .NET 的长进程检查周期默认 24 小时、可配置为 1..168 小时，Android 再次进入前台时最多每天检查一次。管理端概览可查看最近登录上报的客户端版本分布。
