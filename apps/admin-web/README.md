...
specus 管理后台前端,使用 **React + TypeScript + HeroUI + Vite** 实现。构建产物拷入
...
任意一个 specus-server(Go/C#/Java)在 8088 端口跑起来即可联调。
...
npm run deploy:csharp # build 后只写 implementations/csharp/server/src/Specus.Server/wwwroot/
npm run deploy:all   # build 后写入三端静态目录（手动同步用）
```

`dist/` 和三端静态同步目录都是构建产物,已被 `.gitignore` 忽略,只在本地构建/打包时生成,不要提交。

Java `specus-server` 的 Maven 构建已经接入该流程：执行 `mvn package` / `mvn install`
时，会在 `generate-resources` 阶段调用这里的 `npm run deploy:java`，让 Spring 资源复制前拿到
最新管理后台产物，且不会改动 Go/C# 静态目录。需要纯后端构建时可加 `"-Dspecus.server.web.skip=true"` 跳过。

C# `Specus.Server` 的 MSBuild 构建会调用 `npm run deploy:csharp`，可用
`/p:SpecusServerWebSkip=true` 跳过。Go server 没有 `go build` 前置生命周期，使用
...
`/oidc`、`/oidc-config`、`/http`、`/actuator`、`/health`、`/.well-known`、`/ws` 反代到后端 `specus-server:8088`。完整配置见
...
- `implementations/csharp/server/src/Specus.Server/wwwroot/`(下次 `dotnet build`/运行生效)
...
- 客户端消息与附件、免登录房间互传（文件、剪贴板、同步白板；Token 房间支持 OWNER/EDITOR/VIEWER 角色邀请和撤销；白板包含自由绘制与基于 maxGraph + Yjs 的专业流程图模式，支持多页面、分类图形库与模板、动态泳池/泳道、容器与组合、智能参考线、小地图、自动布局、格式刷、高级样式、评论、协作光标、服务端版本历史和只读访客；支持 `.stwb` / `.stdg` / 多页 `.drawio`、Mermaid、PlantUML、Visio `.vsdx` 导入，以及 `.stdg` / `.drawio`、Mermaid、PlantUML、Visio `.vdx`、SVG、PNG、全页 PDF 导出；文件传送按对端网络分策略：同一公网出口的设备（设备列表带"自动发现"徽标）仅走 WebRTC 直连与 TURN 中继、失败不降级；跨网设备在直连/中继失败后，登录且明确授权的用户可回退 OSS 云端接力与分享链接）。
